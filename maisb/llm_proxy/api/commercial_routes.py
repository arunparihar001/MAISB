...
def create_public_api_key(email: Optional[str] = None, company: Optional[str] = None) -> str:
...
@router.post("/v1/profile/signup", tags=["Profile"])
...
@router.get("/v1/profile/verify-email", tags=["Profile"])
...
@router.post("/v1/profile/verify-email", tags=["Profile"])
...
@router.get("/v1/profile/status", tags=["Profile"])
...
@router.post("/v1/profile/api-keys", tags=["Profile"])
def profile_create_api_key(body: ProfileApiKeyRequest) -> Dict[str, Any]:
    ...
    raw_key = create_public_api_key(body.email, profile["company"])
    now = utcnow()
