...
def public_signup(body: PublicSignupRequest, request: Request) -> Dict[str, Any]:
    ...
    conn.commit()
    conn.close()
    send_resend_email("Welcome to MAISB", f"Your MAISB signup is active. API key: {mask_key(raw_key)}", body.email)
    if RESEND_ADMIN_EMAIL:
        send_resend_email("New MAISB signup", f"New signup: {body.email} ({body.company or 'n/a'})", RESEND_ADMIN_EMAIL)
    return {
        "created": True,
        "signup_id": signup_id,
        "api_key": raw_key,
...
def billing_upgrade_request(body: BillingRequest) -> Dict[str, Any]:
    ...
    conn.commit()
    conn.close()
    send_resend_email("MAISB billing request received", f"We received your {body.plan} request ({provider}). Request ID: {request_id}", body.email)
    if RESEND_ADMIN_EMAIL:
        send_resend_email("New MAISB billing request", f"Request {request_id}: {body.email} plan={body.plan} provider={provider}", RESEND_ADMIN_EMAIL)
    return {
...
def paddle_checkout_session(body: PaddleCheckoutRequest) -> Dict[str, Any]:
    ...
    conn.commit()
    conn.close()
    send_resend_email("MAISB checkout started", f"Paddle checkout started for plan={body.plan}. Request ID: {request_id}", email)
...
async def paddle_webhook(request: Request) -> Dict[str, Any]:
    ...
    provider_reference = str(data.get("id") or data.get("subscription_id") or data.get("transaction_id") or custom_data.get("subscription_id") or "").strip()
    status = str(data.get("status") or payload.get("status") or ("paid" if event_type in {"transaction.completed", "subscription.activated"} else "pending"))
...
def certify_start(body: CertifyStartRequest) -> Dict[str, Any]:
    ...
    conn.commit()
    conn.close()
    send_resend_email("MAISB Certify request received", f"Your certify order {order_id} is now in assessment_requested state.", body.email)
    return {
        "order_id": order_id,
