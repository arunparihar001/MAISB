...
export default function SocConsole() {
  ...
  return <div className="stack"><h2>SOC Console</h2><div className="card form-grid"><input type="password" value={adminKey} onChange={(e) => setAdminKeyInput(e.target.value)} placeholder="Admin key" /><button type="button" onClick={load} disabled={loading}>{loading ? 'Loading…' : 'Load SOC data'}</button></div>{error && <p className="error">{error}</p>}{summary && <div className="grid"><MetricCard title="Summary keys" value={Object.keys(summary).length} /><MetricCard title="Risk rows" value={riskRows.length} /><MetricCard title="Cases" value={caseRows.length} /></div>}<RiskQueueTable title="Phase 4 Risk Queue" rows={riskRows} /><RiskQueueTable title="SOC Cases" rows={caseRows} /><RiskQueueTable title="Commercial Admin Requests" rows={commercialRows} /></div>
}
